...
When you are using our CDN, ad-blocking or script-blocking extensions may prevent our SDK from being fetched and initialized properly. Because of this, any call to SDKs API will fail and may cause your application to misbehave.

Additionally, even when the SDK is downloaded and initialized correctly, Sentry endpoints that need to receive captured data may be blocked as well. This will prevent any error reports, sessions health or performance data to be delivered, and effectively not available in your Sentry dashboard.

The first issue can be worked around in multiple ways explained above. However, the endpoint blockage can be only resolved using the tunnel.

### Using the `tunnel` Option

A tunnel is an HTTP endpoint that will act as a proxy between Sentry and your application. Due to the fact that you control this server, there is no risk of any requests sent to that server being blocked. When the endpoint lives under the same origin (although it does not have to in order for the tunnel to work), the browser will not treat any requests to it as a third party requests, and they will have different security measures applied, which ad-blockers does not capture by default. A quick summary of the flow can be found below.
...
To enable the `tunnel` option, provide either a relative or an absolute URL in your `Sentry.init` call:
...
Once configured, all events will be sent to the `/tunnel` endpoint. This solution, however, requires an additional configuration on the server, as the events now need to be parsed and redirected to Sentry. Here is an example code written in PHP:
...
With this set up, events should not be blocked by ad-blockers, as they are never sent to a third-party URL.

If your use-case is related to the SDK package itself being blocked, any of the following solutions will help you mitigate this issue.
